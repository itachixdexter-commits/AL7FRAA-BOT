...
async def get_tiktok_user_info(update: Update, context) -> None:
    ...
    try:
        ...
        if "captcha" in response.text.lower() or "verify you are human" in response.text.lower():
            await msg_wait.edit_text("❌ تم اكتشاف صفحة تحقق (كابتشا/anti-bot). يرجى المحاولة لاحقًا.")
            return
        ...
        if not script_tag or not script_tag.string:
            await msg_wait.edit_text("❌ لم يتم العثور على بيانات الحساب (script tag مفقود أو فارغ). قد يكون الحساب خاصاً أو غير موجود أو تغير هيكل الصفحة.")
            return

        try:
            data = json.loads(script_tag.string)
        except json.JSONDecodeError:
            await msg_wait.edit_text("❌ خطأ في تحليل بيانات JSON من script tag. قد يكون هيكل الصفحة قد تغير.")
            return
        ...
        try:
            ...
            if not u_detail:
                # Alternative structure check for SIGI_STATE or direct webapp.user-detail
                u_detail = data.get("webapp.user-detail", {})
            
            user_info = u_detail.get("userInfo", {})
            if not user_info:
                await msg_wait.edit_text("❌ لم يتم العثور على بيانات المستخدم في الرد. قد يكون اليوزر غير صالح أو الحساب خاص.")
                return
            ...
            await msg_wait.edit_text(f"❌ خطأ في تحليل البيانات المستخرجة: {str(e)}. قد يكون هيكل البيانات قد تغير.")
    except httpx.HTTPStatusError as e:
        if e.response.status_code == 404:
            await msg_wait.edit_text("❌ لم يتم العثور على المستخدم. تأكد من صحة اسم المستخدم.")
        else:
            await msg_wait.edit_text(f"❌ خطأ في طلب HTTP (كود: {e.response.status_code}). يرجى المحاولة مرة أخرى.")
    except httpx.RequestError as e:
        await msg_wait.edit_text(f"❌ خطأ في الاتصال بالإنترنت أو مهلة الطلب: {str(e)}. يرجى التحقق من اتصالك بالإنترنت والمحاولة مرة أخرى.")
    except Exception as e:
        await msg_wait.edit_text(f"❌ حدث خطأ غير متوقع أثناء جلب معلومات تيك توك: {str(e)}")
    finally:
        context.user_data["state"] = None
